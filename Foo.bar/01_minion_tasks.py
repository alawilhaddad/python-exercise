"""
Commander Lambda's minions are upset! They're given the worst jobs on the whole space station, and some of them
are starting to complain that even those worst jobs are being allocated unfairly. If you can fix this problem, 
...
shifts are planned well in advance, the lists for each task will contain up to 99 integers. When a minion is
...
if __name__ == "__main__":
    ids = list(map(lambda x: int(x), input("input data (space separated): ").split( )))
    z = int(input("input limit: "))
    print(solution(ids, z))
